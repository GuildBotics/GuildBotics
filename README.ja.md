...
CLI ベースのチャット返信では、GuildBotics は `~/.guildbotics/data/memory/<person_id>/` に agent ごとの記憶リポジトリも保持します。返信生成はそのディレクトリを `cwd` にして実行されるため、利用している CLI バックエンドは `AGENTS.md`、`CLAUDE.md`、`GEMINI.md` などの標準 instruction file をそのまま利用できます。
