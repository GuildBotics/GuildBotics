...
GuildBotics では、プロジェクトの設定と状態を置くフォルダを **ワークスペース** と呼びます。ワークスペースには、以下のようなプレーンテキストの設定ファイルが書き出されます。
...
- **LLM・AI CLIツール**: デフォルトの LLM、AI CLI ツールの選択、LLM API キーの設定
- **エージェント実行環境**: **ビルド** でこのマシンに環境を作り、表示されるコマンドで選んだツールへログインします。どちらもマシンごとの作業です
...
対話中に、別のメンバーへ単発作業を依頼することもできます。

```text
bob に、未コミットの編集も含めて現在の変更をレビューしてもらってください。
```

対話中のメンバーが組み込みコマンド `ask` を実行し、bob の結果を伝えます。bob は同じ作業ツリーを読み、修正を依頼した場合はその編集も同じ場所へ反映されます。あらかじめ GuildBotics でワークスペースを選び、**設定 → エージェント実行環境**で、このマシン上の bob のエージェント隔離環境と AI CLI のログインを準備してください。対話で使う host 側の CLI ログインとは別です。レビューには数分かかることがあり、隔離環境やログインが使えない場合は、その理由が対話へ返ります。

依頼する前に、同じワークスペースを選んだ GuildBotics Desktop を開いてください。host の `guildbotics run` は利用可能な Desktop で実行されます。Desktop が閉じている場合や別のワークスペースを選んでいる場合は手元で実行します。Desktop へ送った後のエラーや通信切断では手元で再実行せず終了し、メンバーが実行中の場合はその拒否理由を返します。 Desktop 経由では完了時にまとめて結果が返り、途中のログと呼び出し元シェルの環境変数は転送されません。
...
- **巡回実行コマンド**: 各メンバーの巡回作業が残っていない状態で **巡回間隔（分）** が経過したら実行します。チケット駆動ワークフローは候補をすべて処理してから巡回間隔の待機に入ります。GitHub アカウントを設定した新規メンバーの既定は チケット駆動ワークフローです。GitHub を使わないメンバーには巡回実行コマンドが設定されないため、**設定 → メンバー → 巡回** で選んでください
...
- チケットから作成された PR へのレビューは PR 上で書き込んでください。メンバーはチケットではなく Pull Request の巡回でその PR を追います（→ [Pull Request の巡回](#pull-request-の巡回)）
...
巡回のたびに、チケットのレーンを見る前に、プロジェクトのオーナー配下でメンバーが関わっている open な pull request を確認します。チケットは不要で、PR そのものが作業単位です。

- **メンバーが作成した PR**: 最後の発言が他の人の未解決 review thread、review summary、まだ答えていない conversation comment を拾います。メンバーは妥当な指摘についてコードを修正し、各 thread に返信し、コメントに答えます
- **メンバーがレビューする PR**: レビューを依頼されたとき（personal token のメンバーのみ。GitHub Apps にはレビュー依頼ができません）、参加中の thread に自分以外の人から返信があったとき、前回のレビュー後に新しいコミットが積まれたときに再度レビューします。自分以外の人からの thread 返信または新しいコミットによる自動再レビューは 3 回で止まり、メンバーがその旨を PR 上に告知します。その後は明示的なレビュー依頼だけがメンバーに届きます
- 結論は GitHub の review（approve か request changes）として submit します。これがレビュー依頼を消化し、メンバーをその PR のレビュワーにします。最初のレビューは Slack か対話セッションで依頼してください（→ [Slack で作業を依頼する](#slack-で作業を依頼する)）。以降は巡回がその PR を追います
- **Draft の PR はどちらの役割でも巡回しません。** ある PR をしばらく自分の手で進めたいとき（例: 対話セッションで方針を練り直す間、他のメンバーの再レビューを止めたいとき）は、その PR を Draft に変換してください。サービスはそれ以外の作業を続けます。終わったら **Ready for review** に戻すと、次の巡回から再び拾われ、新しいコミットには再レビューが走ります
...
- **巡回実行コマンド**: サービスの稼働中、ラウンドロビンで繰り返し実行します（例: タスクボードの巡回）。**巡回間隔（分）** は巡回作業が残っていないときの待ち時間です。チケット巡回は候補をすべて処理するまで、候補間でこの間隔を待ちません。「このメンバーの巡回実行コマンドを設定する」をオンにして、保存済みコマンドを選びます。オフのメンバーは巡回実行しません
...
guildbotics secrets pull                          # Hub マシンから不足・古い値をまとめて取得
guildbotics secrets push                          # このマシンで入力した値を Hub マシンへ送る
guildbotics secrets export --file secrets.env     # 引越用にシークレットを書き出し
guildbotics secrets import secrets.env            # 移行先マシンで読み込み
```

複数マシンでワークスペースを共有している場合、値は共有履歴には入らず、利用者が送信・取得を実行した
...
5. サーバー上でエージェント隔離環境をビルドし、AI CLI ツールへログインします（サーバーには KVM が必要です。[必要なもの](#必要なもの) を参照）:
   ...
   ```bash
   guildbotics environment status                    # このマシンの runtime・ベースイメージ・環境・ログイン状態
   guildbotics environment image load agent.tar      # 宣言がベースイメージを指すときだけ: このマシン向けに docker save したアーカイブを読み込む
...
ワークスペースを同期している場合、サービスには Hub 上の永続的な owner も 1 台だけ存在します。
`guildbotics start` とデスクトップのサービス画面は、サービスの仕事を受け付ける前に Hub へ接続して
このマシンが owner であることを確認します。別の device が owner なら新しいサービス処理は受け付けず、
owner の変更は共通実行境界で検出して旧サービスを停止します。Hub が一時的に不達でも実行中の仕事は
中断しませんが、owner を再確認できるまで新しいサービス処理は開始しません。

owner は lease ではなく、自動変更されません。旧マシンのサービスが停止したことを直接確認してから、
同期画面の「このマシンへ引き継ぐ」を使います。現在の作業表示は Git 履歴ではない一時的な relay state で、
publisher が heartbeat を止め、状態が失効すると消えます。完了した仕事は共有 TaskRun 記録へ保存されるため、結果の push 後に
サービスが再起動しても同じ入力を再度受け付けません。
...
Hub には現在の service owner と一時的な現在状態も置かれます。owner は永続し、旧サービスが停止したことを
直接確認した後の明示的な引き継ぎでのみ変更されます。Hub が不達の間も実行中のサービスは現在の仕事を続けますが、
owner を確認できるまで新しいサービス処理は開始しません。現在状態は Git へ commit されず、publisher が終了すると
heartbeat が止まり、その状態は失効後に Hub から削除されます。
...
API キーとトークンの**値**は共有されません。マシン間を移動するのは key 名と世代だけで、値そのものは
各マシンの OS 秘密ストアに残ります。Hub マシンの OS 秘密ストアが配布拠点になり、値は利用者が
送信・取得を実行したときだけ、SSH の標準入出力を通って移動します。Git リポジトリ、Hub 上の中継
ファイル、一時ファイルのいずれにも値は書かれません。

一覧と操作は**設定 → 同期**の「このマシンの認証情報」にあります。プロバイダーやメンバーの入力欄には
状態の表示とこの画面への導線だけを置き、転送のボタンは置いていません。

**macOS を Hub にする場合**は、認証情報を送信・取得する前に Hub のユーザーで GUI ログインし、
GuildBotics Desktop を起動してください。起動が必要なのは転送中だけで、開いているワークスペースは
問いません。状態確認を含む Hub Secret コマンドは、認証付きのループバック API を通じて Desktop に
GUI セッション内のキーチェーン操作を委譲します。Desktop が利用できなければ起動を案内します。
Windows と Linux の Hub は OS 秘密ストアを直接利用します。

**新しいマシンを追加したとき**は「まとめて取得」を 1 回選ぶだけです。このマシンに無い値と古い値が
まとめて届き、そのままこのマシンの OS 秘密ストアへ登録されます。値を入力し直す必要はありません。

**値を更新したとき**は、入力したマシンで「Hub マシンへ送る」を選びます。送信の直前に共有ファイルを
取り込むため、しばらくオフラインだったマシンでも古い世代を基準に送ることはありません。送信が完了すると
共有世代が上がり、他のマシンには「値が更新されています」と表示されます。各マシンで「取得」を選ぶと
最新の値に揃います。

「まとめて送る」は、Hub マシンがまだ持っていない値をすべて対象にします。既存のワークスペースを
Hub へ接続した直後や、Hub を作り直した直後は、この 1 回で全部の値が Hub へ渡ります。

2 台で同じ key を変更すると、後から受け取った側に「2 台で同じ値を変更しました」と表示されます。
残したい方をその key の行で選びます。「送る」ならこのマシンの値が、「取得」なら相手のマシンの値が
新しい世代になり、もう一方のマシンにも通知されます。入力し直す必要はありません。「まとめて送る」
「まとめて取得」はこの状態の key を対象にしません。どちらを残すかは 1 件ずつの判断だからです。

Hub 側に値が保存された直後に共有世代の記録が中断した場合は「確認が必要です」と表示されます。
値を持っているどのマシンからでも「Hub マシンへ送る」を選べば解消します。送信は Hub が持っている
世代の次に乗るため、中断した送信より先へ進んで記録が確定します。送信元のマシンが手元に無くても、
そのマシンで値を入力し直したあとでも同じです。

この状態のあいだ、その key の「取得」は行えません。共有記録に無い世代を取り込むと、確認が必要だと
示している手がかり自体が消えてしまうためです。

デスクトップアプリを動かさないマシンでは CLI を使います。

```bash
guildbotics secrets status   # このマシンの状態と、対応が必要な key
guildbotics secrets pull     # 不足・古い値をまとめて取得する
guildbotics secrets push     # このマシンで入力した値を Hub マシンへ送る
```

OS 秘密ストアがロックされている間は、送信も取得もできません。GuildBotics が平文のファイルへ
書き出すことはないため、ロックを解除してからやり直してください。

**ヘッドレスの Linux をデバイスとして使う場合**は、OS 秘密ストア（Secret Service）が
...
マシンを紛失したときは、そのマシンの公開鍵を Hub から削除します。

1. **設定 → 同期**の device 一覧で、対象マシンの SSH 公開鍵 fingerprint を確認します。
2. Hub マシンで、その fingerprint に対応する行を `~/.ssh/authorized_keys` から削除します。

   ```bash
   ssh-keygen -l -f ~/.ssh/authorized_keys
   ```

   GuildBotics は `authorized_keys` を編集しません。どの行を消すかの判断と操作は利用者が行います。

3. そのマシンに既に保存されている値は遠隔からは消せません。GitHub や Slack、LLM プロバイダー側で
   トークンと API キーを失効させ、再発行してください。再発行した値は、手元のマシンで入力してから
   「Hub マシンへ送る」で他のマシンへ配ります。
...
| Hub 同期エラー   | ローカル利用は可能、共有は遅延                 | 表示される「エラー内容」を確認し、原因を解消して「再試行」。SSH が原因なら [1 台目を設定する](#1-台目を設定する)の前提条件を確認 |
...
- `intelligences/brain_mapping.yml`: 機能ごとのエンジンと LLM／AI CLI スロットの割り当て。チャットの起動要否を判定する `chat_decision` の設定は、[Slack 連携ガイド](docs/slack_integration.ja.md#チャット判断エンジン)を参照してください。
...
| AI CLI の turn が始まらない・「この端末では実行できません」と警告が出る | **設定 → エージェント実行環境** の **この端末の状態**（または `guildbotics environment status`）を確認してください。runtime にはハードウェア仮想化（Apple Silicon / Windows ハイパーバイザー プラットフォーム / KVM）が必要で、環境が **準備完了**（**ビルド** を押す。サービス稼働中は宣言の変更で自動再ビルド）、ツールの認証情報がこのマシンに保存されている必要があります（画面のログインコマンドを実行）。保存済みは認証の成功を保証しません。ビルド中は作業が失敗ではなく見送りになります |
| 認証情報保存済みなのに AI CLI の認証に失敗する | **設定 → エージェント実行環境** で対象ツールのログインコマンドをコピーし、ターミナルで実行してください。画面を開いている間、状態は10秒ごとに自動更新されます。保存状態を読み直すには **表示を更新** を押します。利用レートの再取得には、対応ツールの **再確認** を使います。macOS / Linux は Desktop 管理 CLI の絶対パス、Windows は PATH 上の `guildbotics` を使います。**認証情報保存済み** はファイルの存在だけを表します（Claude Code は、この端末のキーチェーンで暗号化保存したログインを開けること。Claude Code のログインは turn の microVM に入らず、以前の平文ファイルのログインは読まないため、更新後に一度 Claude Code へログインし直し、旧ファイルを削除してください。詳しくは[こちら](docs/native_agent_runtime.ja.md#vmの外で管理するログインclaude-code)）。認証失敗は同じ端末・同じツールを使う全メンバーへ反映され、ログイン完了または後続の正常実行で解除されます。ネットワーク障害やレート制限は認証結果を変えず、過去の失敗が再試行を妨げることもありません |

AI CLI の利用レート取得に失敗すると、画面上部と設定の対象行に同じ問題を表示します。対象行には最終確認時刻も表示します。**再確認** は対象ツールのキャッシュを使わずに取得し、**エラー詳細** は該当する診断セッションを開きます。再取得に成功すると通知と対象行の警告が解除されます。この通知だけで認証期限切れと断定するものではありません。通信障害や利用枠を表示できない認証方式でも取得に失敗するためです。**表示を更新** は保存状態を読み直し、ツールへの再取得は行いません。アクティビティ画面ではツールごとに個別に取得し、同じツールを使うメンバー間で結果を共有するため、遅いツールが他のツールの表示を待たせません。更新中も前回取得できた値を取得時刻つきで表示し続けます。更新に失敗した場合は前回の値であることを示し、その値では Rate limit 表示を判定せず、通常の5分ではなく30秒後に再取得します。
