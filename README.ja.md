...
対話中のメンバーが組み込みコマンド `ask` を実行し、bob の結果を伝えます。bob は同じ作業ツリーを読み、修正を依頼した場合はその編集も同じ場所へ反映されます。あらかじめ GuildBotics でワークスペースを選び、**設定 → LLM・AI CLIツール**で、このマシン上の bob のエージェント隔離環境と AI CLI のログインを準備してください。対話で使う host 側の CLI ログインとは別です。レビューには数分かかることがあり、隔離環境やログインが使えない場合は、その理由が対話へ返ります。
...
- AI CLI の turn は、サービスを実行するマシンのエージェント隔離環境の中で動きます。**設定 → LLM・AI CLIツール** で環境が **準備完了**、ツールが **この端末に認証情報保存済み** になっていることを確認してください。ターミナルからは次で確認できます:
...
| AI CLI の turn が始まらない・「この端末では実行できません」と警告が出る | **設定 → LLM・AI CLIツール** の **エージェント隔離環境** カード（または `guildbotics environment status`）を確認してください。runtime にはハードウェア仮想化（Apple Silicon / Windows ハイパーバイザー プラットフォーム / KVM）が必要で、環境が **準備完了**（**ビルド** を押す。サービス稼働中は宣言の変更で自動再ビルド）、ツールの認証情報がこのマシンに保存されている必要があります（カードのログインコマンドを実行）。保存済みは認証の成功を保証しません。ビルド中は作業が失敗ではなく見送りになります |
| 認証情報保存済みなのに AI CLI の認証に失敗する | **設定 → LLM・AI CLIツール** で対象ツールのログインコマンドをコピーし、ターミナルで実行してください。カードを開いている間、状態は10秒ごとに自動更新されます。保存状態を読み直すには **表示を更新** を押します。利用レートの再取得には、対応ツールの **再確認** を使います。macOS / Linux は Desktop 管理 CLI の絶対パス、Windows は PATH 上の `guildbotics` を使います。**認証情報保存済み** はファイルの存在だけを表します。認証失敗は同じ端末・同じツールを使う全メンバーへ反映され、ログイン完了または後続の正常実行で解除されます。ネットワーク障害やレート制限は認証結果を変えず、過去の失敗が再試行を妨げることもありません |
