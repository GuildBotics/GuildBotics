...
デスクトップアプリには CLI が同梱されており、初回起動時に CLI エージェントから参照しやすい場所へ管理用 CLI とスキルを配置します。
...
これらはすべてプレーンテキストの設定ファイルです。
GuildBoticsの設定はすべて上記のファイルに保存されるため、GUI のない環境（サーバー等）へワークスペースフォルダをコピーすれば、`guildbotics` CLI だけで運用できます。
...
- **LLM・CLIエージェント:** デフォルトのLLM、CLIエージェントの設定とLLM API Key設定
- **メンバー:** チームメンバーの追加と設定
...
- 実行時の診断ログは、既定で `<workspace>/.guildbotics/data/run/diagnostics.jsonl` に記録
...
Slackチャットワークフローでは、`person.yml` の `message_channels` に設定したチャネルを監視し、受信イベントを設定済み CLI エージェントへ委譲します。CLI エージェントは返信、リアクション、no-op、質問、blocked のいずれにするかを判断します。Slack への投稿・返信・リアクションは公開メンバー機能である `guildbotics member chat ...` 経由でのみ実行されます。
...
CLI エージェントによるチャット処理では、`functions/handle_chat_event` がエージェントごとの作業ディレクトリを `cwd` にして実行されます。既定では `<workspace>/.guildbotics/data/workspaces/<person_id>/` です。この配下にある複製済みリポジトリを参照できます。ワークフローは `guildbotics member chat complete` が記録した実行証跡を検証し、エージェントの自然言語の標準出力だけでは Slack に投稿した証拠として扱いません。
`person.yml` の `character` には、興味・嗜好・会話参加方針などを定義できます。チャット判断と返信生成は CLI エージェント経由でこのプロフィールを参照します。
...
- チケット駆動の作業は `guildbotics member ...` CLI 経由で行います。ワークフローは GitHub Project
  から対象を選び、CLI エージェントをメンバーごとの作業ディレクトリで起動します。既定では
  `<workspace>/.guildbotics/data/workspaces/<person_id>` です。エージェントが完了記録を残したことを
  検証し、リポジトリの複製、push、PR 作成、コメント、返信はエージェント自身が
...
- 各 AI メンバーの GitHub 認証情報を GuildBotics に設定してください。GitHub/git 書き込みは
  ...
  使用中のワークスペースの `.env`、`GUILDBOTICS_ENV_FILE`、またはカレントディレクトリの `.env` からこれらを読み込みます。
- CLI エージェントを対話的に使う場合は、ワークスペース選択後に GuildBotics デスクトップアプリを
...
**診断ログ**: 実行時のイベントやエラーは、既定で `<workspace>/.guildbotics/data/run/diagnostics.jsonl` に記録されます。デスクトップアプリの診断画面からも確認できます。
