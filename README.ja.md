...
明示的なセットアップ手順は不要です。GuildBotics は以下のフィールドを管理します:
- `Mode`: 動作モード（comment/edit/ticket）
- `Role`: タスク実行時の役割
- `Agent`: タスクを実行するAIエージェント

**ステータスマッピング**: GuildBotics は GitHub Projects のステータスを自身のステータス
（New/Ready/In Progress/In Review/Retrospective/Done）にマッピングします。GitHub Project で独自の
ステータス名を使っている場合は、`team/project.yml` の `services.ticket_manager.status_map` キーで
マッピングを指定してください（[設定ファイル](#72-設定ファイル)を参照）。
...
- `services.ticket_manager.status_map`: GuildBotics のステータス（New/Ready/In Progress/In Review/Retrospective/Done）を GitHub Project のステータス名に対応付けます。Project が独自のステータス名を使う場合に設定します。
