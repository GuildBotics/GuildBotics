...
- チケット駆動の作業は `guildbotics member ...` CLI 経由で行います。workflow は GitHub Project
  から対象を選び、CLI エージェントを `~/.guildbotics/data/workspaces/<person_id>` で起動し、
...
**メモ:**
AIエージェントは `guildbotics member git prepare` で `~/.guildbotics/data/workspaces/<person_id>` 配下に repository を準備し、そこで作業します。
