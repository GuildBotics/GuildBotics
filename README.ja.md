...
デスクトップアプリには CLI が同梱されており、初回起動時に CLI agent から参照しやすい場所へ managed CLI と skill を配置します。
...
# 2. デスクトップアプリが配置した managed CLI で実行する
...
**デスクトップアプリ（セットアップ + managed CLI）:** GuildBotics デスクトップアプリをビルド／インストールします。
...
- active workspace を `~/.guildbotics/data/active-workspace.json` に記録
- 言語選択（英語/日本語）
...
active workspace により、`.env` や `.guildbotics/config` が存在しない CLI agent の作業ディレクトリからでも `guildbotics member ...` が同じ設定を参照できます。CLI から確認・変更するには以下を使います。
...
GuildBotics は machine state と workspace data を分離します。active workspace state と CLI
scheduler の PID は `$HOME/.guildbotics/data` 配下に保存され、workspace `.env` の影響を受けません。
member workspace、task/chat run evidence、diagnostics、prompt trace、chat state などの runtime data は、
既定で `<workspace>/.guildbotics/data` に保存されます。`GUILDBOTICS_DATA_DIR` はこの workspace data root
だけを上書きし、active workspace state や scheduler PID の保存先は変えません。`GUILDBOTICS_CONFIG_DIR`
は config source の指定であり、workspace root としては扱いません。
...
- runtime diagnostics は `<workspace-data-root>/run/diagnostics.jsonl` に記録
...
CLI agent によるチャット処理では、`functions/handle_chat_event` が agent ごとの workspace root である `<workspace-data-root>/workspaces/<person_id>/` を `cwd` にして実行されます。この配下にある cloned repository を参照できます。workflow は `guildbotics member chat complete` が記録した run evidence を検証し、agent の自然言語 stdout は Slack side effect の証拠として扱いません。
...
## 6.1. 事前準備
### 6.1.1. Git環境
- チケット駆動の作業は `guildbotics member ...` CLI 経由で行います。workflow は GitHub Project
  から対象を選び、CLI エージェントを `<workspace-data-root>/workspaces/<person_id>` で起動し、
  agent が task completion を記録したことを検証します。clone / push / PR / comment / reply は
  agent 自身が `guildbotics member` 経由で実行します。
...
- CLI agent を対話的に使う場合は、ワークスペース選択後に GuildBotics デスクトップアプリを
  ...
  exfiltration を完全に技術的に封じ込める sandbox ではありません。
...
**メモ:**
AIエージェントは `guildbotics member git prepare` で `<workspace-data-root>/workspaces/<person_id>` 配下に repository を準備し、そこで作業します。
...
- **AIエージェントの実行結果をタスクボード上で確認**
  - AIエージェントがタスクを完了すると、member capability 経由で comment / PR / review reply / reaction のいずれかの痕跡を残します
- **AIエージェントによるPull Requestの作成**
  - コード変更が必要な場合、agent が member workspace の branch を publish し、`guildbotics member github pr create` で Pull Request を作成または再利用します
- **チケット作成**
...
`guildbotics member` command は、まず `--workspace <dir>` を優先します。指定が無い場合、
明示的な `GUILDBOTICS_CONFIG_DIR` やカレントディレクトリの `.guildbotics/config` が既にあればそれを維持し、
それも無ければ desktop app または `guildbotics workspace use` が記録した active workspace を使います。
選択された workspace から `GUILDBOTICS_CONFIG_DIR` を設定し、`<workspace>/.env` が存在する場合は
`GUILDBOTICS_ENV_FILE` も設定します。

workspace の確認・変更には以下を使います。
...
デスクトップアプリを使わない headless 運用では、絶対パスの `.env` を指す `GUILDBOTICS_ENV_FILE`、
またはカレントディレクトリの `.env` がフォールバックになります。`guildbotics start` と desktop runtime は
workspace の `.env` を読み込んだときに `GUILDBOTICS_ENV_FILE` も自動設定します。
workspace `.env` に `GUILDBOTICS_DATA_DIR` を設定すると workspace data root を上書きできます。
process 起動時点で `GUILDBOTICS_DATA_DIR` が設定されており、workspace `.env` に override が無い場合は、
その process の共有 fallback data root として使われます。
...
**Diagnostics**: runtime event や error は `<workspace-data-root>/run/diagnostics.jsonl` または Desktop の diagnostics view で確認してください。
