...
- **メンバーがレビューする PR**: レビューを依頼されたとき（personal token のメンバーのみ。GitHub Apps にはレビュー依頼ができません）、参加中の thread に返信があったとき、前回のレビュー後に新しいコミットが積まれたときに再度レビューします。新しいコミットに対する再レビューは 3 回で止まり、メンバーがその旨を PR 上に告知します。その後は thread への返信か明示的なレビュー依頼だけがメンバーに届きます
