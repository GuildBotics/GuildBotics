...
- **`.env` フォールバック:** インデックスファイルの無いワークスペース（この機能より前に作成した
  すべてのワークスペースを含む）は、従来どおりワークスペースの `.env` を使い続けます。ヘッドレス
  サーバーや CI ではこの方式を引き続きサポートします。GuildBotics が書き出す `.env` は
  所有者のみ読み書き可能（`0600`）のパーミッションになります。
- **優先順位:** 実環境変数 > OS キーチェーン > `.env`。サーバー運用では、バックエンドに関係なく
  環境変数での注入が常に最優先されます。
- **GitHub App 秘密鍵:** `guildbotics secrets migrate`（およびキーチェーン利用ワークスペースでの
  メンバー設定保存）は、`*_GITHUB_PRIVATE_KEY_PATH` が指す PEM ファイルの中身をキーチェーンへ
...
シークレットの管理には `guildbotics secrets` CLI を使います。

```bash
guildbotics secrets status                 # 使用中のバックエンドとキーチェーンの利用可否
guildbotics secrets migrate                # .env のシークレットを OS キーチェーンへ移行
guildbotics secrets list                   # 保存済みキー名の一覧（値は表示しない）
guildbotics secrets set KEY                # シークレットを追加・更新（非表示プロンプト）
guildbotics secrets delete KEY             # シークレットを削除
guildbotics secrets export --file s.env    # 引越し用に dotenv 形式でエクスポート
guildbotics secrets import s.env           # 移行先マシンでインポート
```
