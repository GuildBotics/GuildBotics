...
- **メンバー**: チームメンバーの追加と設定
- **GitHub**: GitHub 連携の設定（GitHub を利用する場合のみ）
...
Claude Code や Codex などのセッションにメンバーを呼び出し、いま開いているリポジトリで一緒に作業できます。
...
作業したいリポジトリで AI CLI ツールを起動し、メンバーとしての作業を依頼します。
...
**一歩進んだ例**: Markdown コマンドは、フロントマターでテンプレートエンジンや子コマンド（`commands:`）を指定できます。以下は、OS の表示言語を子コマンドで取得し、翻訳方向を切り替える例です（`.guildbotics/config/commands/translate.md`）。
