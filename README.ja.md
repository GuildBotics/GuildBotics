...
1. Hub 役にするマシンで「このマシンを Hub にする」を選びます。そのマシンで OpenSSH サーバーが
   動いていることを確認してください（[必要なもの](#必要なもの)を参照）。ここで作られるのは
   `~/.guildbotics/hub/` と `hub.json` です。ワークスペース用の repository は、最初のマシンが
   登録したときに作られます。
2. ワークスペースを持つマシンで、「この device の SSH 鍵」の「鍵を作成する」を選び、表示された
   公開鍵をコピーします。鍵は `~/.ssh/id_ed25519` で、同名の鍵が既にあればそれをそのまま使い、
   作り直しません。
...
5. デスクトップアプリに戻り、Hub のアドレスを `user@host` の形で入力し「確認する」を選びます。
