...
**既存の Hub には、この更新後に認証情報を再送してください。** Hub のキーチェーンには値と世代を
一緒に保存するようになりました。採用したい値を持つマシンから、各 key の「送る」または
`guildbotics secrets push KEY` を実行してください。ワークスペース自身の保存済み認証情報は
変わりません。Hub の旧形式の値は読み取りません。
