...
| 認証情報保存済みなのに AI CLI の認証に失敗する | **設定 → LLM・AI CLIツール** で対象ツールのログインコマンドをコピーし、ターミナルで実行後に **状態を更新** を押してください。コマンドには Desktop 管理 CLI の絶対パスが入ります。**認証情報保存済み** はファイルの存在だけを表します。認証失敗は同じ端末・同じツールを使う全メンバーへ反映され、ログイン完了または後続の正常実行で解除されます。ネットワーク障害やレート制限は認証結果を変えず、過去の失敗が再試行を妨げることもありません |
