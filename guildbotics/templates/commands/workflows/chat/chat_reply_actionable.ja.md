...
- `関連メモリー` は最新メッセージに明確に関係するときだけ使い、想起された決定事項・未決事項・source・scope に基づいて返答する。ただし memory 全文をそのまま貼らない
- `関連メモリー.items[].retention.kind` が `transition` の item は変更履歴として扱い、現在方針そのものとしては使わない。現在方針は `current_fact`、`open_question`、または kind 未指定の item を優先する
- `関連メモリー.items` が空の場合、過去 memory を参照できたかのようには書かない
