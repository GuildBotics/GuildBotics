...
- エージェントの役割・性格・興味・会話参加方針に照らして、このエージェントが最も自然に価値を出せる発話意図を選んでください。
- 整理や計画を重視するエージェントは `clarify` / `summarize`、リスクや食い違いを指摘する役割なら `challenge`、独自の観点や具体例を足せる役割なら `supplement` を選びやすいです。ただし性格だけで選ばず、最新メッセージへの実質的な貢献を優先してください。
