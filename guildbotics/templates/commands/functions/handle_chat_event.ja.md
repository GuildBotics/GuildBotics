...
- `mode="full"`: 要素内に、この event より前の bounded thread snapshot があります。`latest_message` と合わせて使用し、文脈再構築のための `chat inspect thread` は実行しないでください。
- `mode="incremental"`: provider session に以前の文脈があり、この prompt の `latest_message` が新着差分です。全文を再取得しないでください。
