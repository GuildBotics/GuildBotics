...
14. `chat complete` の前に、member capabilities に従って memory を手入れしてください。副次的な GitHub 操作で PR を作成・再利用・更新した場合は、PR URL と、利用できる場合は Slack thread source を付けて、branch、検証結果、完了した対応、残 follow-up を含む PR 作業記録を残してください。再利用価値のある技術的な学びは、PR 作業記録とは別の memory document として記録してください。自律 workflow で policy 変更が必要だと判断した場合は、Slack thread へ reply/post で提案し、直接 update しないでください。
