...
6. 作業種別 `pull_request_feedback`: これはあなた自身の PR で、まだ答えていない review thread・review summary・conversation comment があります。member capabilities の標準作業手順「レビュー指摘への対応」に従い、そのすべてに対応してください。妥当な修正を publish し、各 thread には `pr reply` で返信し（返信が不要なら `reaction add` で reaction）、conversation comment には `pr comment` で答えてください。最後に手順 4 と同じく `pr checks` で確認してください。
