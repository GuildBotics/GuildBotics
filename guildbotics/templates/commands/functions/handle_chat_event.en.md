...
9. If a text reply is appropriate, run `guildbotics member chat reply --person {person_id} --service {service_name} --channel-id {channel_id} --thread-ts {thread_ts} --content-stdin` and pass the entire body on stdin. Prefer a quoted heredoc (`<<'EOF'` ... `EOF`) so `$`, backticks, and `$()` remain literal.
...
12. If no post or reaction is needed, run `guildbotics member chat noop --person {person_id} --run-id {workflow_run_id} --service {service_name} --channel-id {channel_id} --thread-ts {thread_ts} --event-id {event_id} --content-stdin` and pass the reason on stdin using a quoted heredoc (`<<'EOF'` ... `EOF`).
...
15. Finish by running `guildbotics member chat complete --person {person_id} --run-id {workflow_run_id} --service {service_name} --channel-id {channel_id} --thread-ts {thread_ts} --event-id {event_id} --status done|asking|blocked --content-stdin` and pass the run summary on stdin using a quoted heredoc (`<<'EOF'` ... `EOF`).
