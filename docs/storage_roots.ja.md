...
Config root は `GUILDBOTICS_CONFIG_DIR` が設定されていればその値、未設定なら cwd の `.guildbotics/config` である。

`$HOME/.guildbotics/config` は暗黙の fallback としては扱わない。ホーム配下などワークスペース外の共有 config を使う場合は、`GUILDBOTICS_CONFIG_DIR` にその directory を明示的に設定する。

Package template は config root に対象ファイルが存在しない場合の fallback として扱う。
