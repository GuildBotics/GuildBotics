...
起動し、コマンド実行が終わると（成功・失敗・キャンセルを問わず）破棄します。ターンは1つずつ順に
動き、先のターンが残したものが見えます（コマンドとそのサブコマンドは同じ隔離の範囲です）。
コマンド実行の外のターン（Desktopのアシスタント）は、ターンごとに自分のmicroVMを起動し、終わると
破棄します。起動後のmicroVMの構成は変えません。起動時にマウントした範囲の外を作業ディレクトリに
したターンや、別のアクセス契約を求めるターンは拒否します。設定されているがログインしていない
AI CLIツールは、そのツールのターンが来るまで何も止めません。
...
プロバイダの設定やskillはsnapshot側のもので、microVMを破棄するたびに元へ戻ります。
...
- **ゲートウェイ**: そのturnの置換用の値を持つ`POST /v1/messages`と
  ...
  upstreamへ届く前に拒否します。ゲートウェイのポートはmicroVMの起動時に開くため、ゲートウェイは
  microVMと同じだけ動きますが、貸し出すのは一度に1つのturnだけです。turnが終わるとその置換用の値は
  何にも使えなくなり、次のturnには改めてログインと置換用の値を貸し出します（あるturnで使えなかった
  ログインの失敗は、次のturnに持ち越しません）。microVMが止まるとゲートウェイも止まります。
  upstreamが401を返したら1回だけ更新して再送します。応答のヘッダーと本文に
  ...
  upgradeしません（通常のHTTP要求として扱います）。転送しなかった経路は
  `METHOD /path`だけを、同じ経路はturnごとに1回だけログに残します。カタログの経路は
...
- **Antigravity固有**: Cloud Code APIはHTTPSでしか受け付けないため、Antigravityのゲートウェイは
  ...
  ゲートウェイは転送しなかった経路を`METHOD /path`だけ、
  同じ経路はturnごとに1回だけログに残します（トークン・query・本文は残しません）。
