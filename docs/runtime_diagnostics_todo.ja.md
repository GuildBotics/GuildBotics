...
- 永続化＋クエリ: `guildbotics/app_api/diagnostics_store.py`、API は `GET /diagnostics/traces`・`GET /diagnostics/traces/{id}`・`DELETE /diagnostics/traces/{id}`・`GET /diagnostics/global`
...
- **LLM / CLI Agent フィルタをログにも拡張**: `span_scope` の名前をレコードに乗せ（`correlation_fields().span`）、ブレインの span を自身のログ出力も含む範囲へ拡張。フィルタは「prompt_trace の `llm.*`/`cli_agent.*`」に加え「`span` が一致する `kind=log`」も対象（CLI agent のロガー出力は `LOG_LEVEL=DEBUG` で出力される）。
...
- `span_id`: trace 内の個別処理（workflow / LLM / CLI Agent 呼び出しなど）を表す。
...
- trace 内の workflow / LLM / CLI Agent 呼び出しは span とし、`parent_id` で親子関係を持つ。
...
- 永続化レコードでは prompt / response 本体を `call_id` 参照の重量ペイロードとして骨格（event / log）から分離保存し、独立に削除できるようにする。retention / 手動削除で機密ペイロードだけを purge できる。
- 初期は自動マスキングを入れない（ローカル開発ツール前提）。GUI に閲覧 / エクスポート / 削除を用意し、保持期間・サイズ上限で露出を抑える。
...
- [x] チャット応答入力、LLM、CLI Agent、command event を同じ trace 詳細に統合表示できるデータ化（query 時に `trace_id` でマージ）。
...
- [ ] 機密ペイロードのエクスポート専用 API（閲覧＝詳細 drawer、削除＝`DELETE /diagnostics/traces/{id}`・prompt-trace 無効化は実装済み。エクスポートは未提供）。
