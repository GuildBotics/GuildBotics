...
- The member worker selects each queued event before any workflow starts. Edits and deletions, the member's own messages, and messages the participation conditions exclude are only marked as read and leave no execution in Diagnostics. The remaining events go to the chat judgment engine, and reaction-only and no-action outcomes are completed right there. The chat workflow (`workflows/chat_conversation_workflow`) starts only for events that need the response agent, so a same-named file in the workspace replaces the response turn, not the selection or judgment
...
- Replies, reactions, no-ops, and completions are recorded through `guildbotics member chat reply|post|reaction add|noop|complete`. GuildBotics verifies these execution records and does not accept the tool's natural-language stdout alone as evidence that something was posted to Slack
