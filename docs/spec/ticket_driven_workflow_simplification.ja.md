...
着手可能 lane でも完了 lane でもない lane は、原則として作業中 lane とみなす。ただし、`Backlog` のような未着手 lane を誤って拾わないように、実装では「GuildBotics が一度着手したチケット」または「担当 agent への明示 mention / 未対応コメントがあるチケット」に限定して判定する。
...
作業記録は最終的に personal memory 連携の材料にもできる。ただし #170 の初期実装では、memory update まで自動化しない。まずは引き継ぎ可能なファイルを確実に残す。
