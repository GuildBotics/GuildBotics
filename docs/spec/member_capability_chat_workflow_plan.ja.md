...
`chat_conversation_workflow` は、Slack event の受信、返信要否判定、返信生成、Slack 投稿、reaction、thread state 更新、personal memory 更新を 1 つの Python workflow 内で実行している。
...
incoming Slack event
  -> chat workflow
     ...
     -> chat workflow updates local state and memory
```

3 者の関係は次のように固定する。

- `chat workflow` は orchestration layer である。Slack event の配送、重複防止、prompt payload 構築、CLI agent 起動、run completion 検証、local state / memory 更新を担当する。
- `CLI agent / skill` は会話判断 layer である。member context、thread context、memory context、profile を読んで、reply / reaction-only / no-op / asking / blocked を判断する。
...
- Slack への副作用が成功した事実は member chat capability が evidence として記録し、workflow はその evidence を読んで processed event / thread state / memory を更新する。
...
chat workflow
  ...
  -> thread state / memory context / participant labels を組み立てる
  ...
  -> chat_reply / chat_post の投稿本文に基づき workflow 側で personal memory を更新する
...
- `guildbotics/templates/commands/workflows/chat_conversation_workflow.py`: 現行 chat workflow。Slack event 処理、返信生成、投稿、memory update が集中している。
...
### memory 更新は workflow 側に残す

Personal memory は chat 返信の UX からは一段外側の永続知識管理である。

- Agent prompt には workflow が recall した `memory_context` を渡す。
- `chat_reply` / `chat_post` の投稿本文がある場合だけ、投稿後の memory update / retention gate / transition memory 作成を workflow 側で行う。
- `chat_reaction` / `chat_noop` だけで完了した run では memory update を行わない。
- `guildbotics member memory ...` や `member chat memory update` は今回の主対象にしない。
- 将来 memory capability を作る余地は残すが、今回の chat member capability 実装とは分離する。
...
- thread context の維持。
- memory recall と `memory_context` の prompt 入力化。
- CLI agent 起動。
...
- latest message / thread messages / thread context / memory context の理解。
...
Workflow は routing に必要な incoming event metadata と memory context だけを prompt payload に渡し、Slack thread 本文は prompt に詰め込まない。interactive / non-interactive のどちらでも、thread 本文は agent が `member chat inspect thread` 経由で取得して判断する。
...
- 投稿後 memory update は既存ロジックを再利用しつつ、入力を evidence 由来の reply text に合わせる。
...
- Workflow payload は routing に必要な incoming event metadata と memory context に絞り、Slack thread 本文は渡さない。
...
- Memory update は投稿後に workflow 側で動く。
...
目的: Slack incoming event が `chat_conversation_workflow -> CLI agent -> guildbotics member chat inspect thread -> guildbotics member chat ... -> member chat complete` の経路で処理され、workflow が run evidence だけを正として local state / memory を更新することを確認する。
...
- Memory update は `chat_reply` / `chat_post` の投稿本文がある場合だけ実行され、`chat_reaction` / `chat_noop` だけでは実行されない。
