...
何も変更しないコマンドは、メタデータで `read_only: true` を宣言できます（Python コマンドでは `COMMAND_METADATA` に `"read_only": True`）。宣言はコマンドのもので、そのコマンドの実行のAI CLIツールのターンは、サブコマンドのものも含めてすべて隔離環境の中で読み取り専用に閉じ込められます（作業ディレクトリも含め、ホストから見えるものはすべて読み取り専用になり、ネットワークはプロバイダのAPIだけに届きます）。そのかわり、実行中の手動コマンドやそのメンバーの巡回実行と並んで動かせます。
...
決めるのは実行したコマンドの宣言だけで、サブコマンド自身の宣言は見ません。読み取り専用のコマンドの中から、別の宣言を持つコマンドを `run_command` などで実行することはできません（拒否されます）。隔離環境が閉じ込めるのはAI CLIツールのターンだけです。コマンド自身のシェルスクリプトや Python のコードはホストで動くので、`read_only: true` を宣言するならそれらも何も変更しないようにしてください。
