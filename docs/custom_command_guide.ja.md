...
- **チャットワークフロー**: 判断エンジンが未処理バッチごとに、対応エージェントの起動・リアクション・行動なしを選びます。会話処理のモデルや effort は指定せず、`functions/handle_chat_event` とその割り当て先の設定を使います。

判断エンジン自身も既存の LLM／AI CLI スロットの設定を使います。判断専用の effort 設定や、スレッド単位の effort 保存はありません。

デスクトップの「チャット判断エンジン」で `chat_decision` に Jev または既存の LLM／AI CLI スロットを割り当てます。不明・情報不足・判定失敗の場合も、会話処理の設定を変えずに対応エージェントへ判断を委ねます。規則、設定手順、評価記録は[チャット判定](chat_decisions.ja.md)を参照してください。
