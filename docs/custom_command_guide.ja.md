...
- **チャットワークフロー**: 選択した判断エンジンが、未処理バッチごとに参加・リアクション・effort を一括で評価します。ファイル作業、リポジトリ調査、規約確認を伴う issue 起票や設計判断は `high`、3項目とも false なら `default`、不明や判定失敗なら `high` です。`low` は明示指定用に残ります。

スレッドに保存済みの `high` は維持します。既に `high` でも参加とリアクションの判断は行います。リアクションのみ・行動なしで終わる場合は effort を昇格させません。状態は person_id ごとに保存し、そのメンバーから見たスレッドの状態を表します。

デスクトップの機能別設定 `chat_decision` で、Jev または既存の LLM／AI CLI スロットを選べます。認証情報がない場合や設定が不正な場合は、対応エージェントへ `high` で委ねます。別の判断エンジンへ自動では切り替えません。規則、設定手順、評価記録は[チャット判定](chat_decisions.ja.md)を参照してください。
