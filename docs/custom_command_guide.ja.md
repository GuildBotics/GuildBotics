...
デスクトップのチャット判断設定で、Jev・Agno のプロバイダー・Claude Code を選べます。認証情報がない場合や設定が不正な場合は、対応エージェントへ `high` で委ねます。別の判断エンジンへ自動では切り替えません。規則、設定手順、評価記録は[チャット判定](chat_decisions.ja.md)を参照してください。
