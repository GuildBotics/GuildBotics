...
- [ ] **S23 — Tauri ネイティブ最小ティア（tauri-driver, workflow_dispatch 想定）** ⏸ 保留（実 OS + Tauri runtime が必要）
  ...
  - 範囲: 「Integration / E2E」§`Tauri ネイティブ smoke（tauri-driver）` + 「E2E / packaging」§`macOS / Linux Tauri smoke`
...
- LLM provider / AI CLIツール選択と API key availability。
...
- team update が model / AI CLIツール / brain mapping files を書く。
...
- AI CLIツール mapping と executable name の対応。
...
### P2: macOS / Linux Tauri smoke（→「Tauri ネイティブ smoke」へ統合）
