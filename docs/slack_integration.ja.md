...
メンバーは、設定で指定したチャネルを監視します。受信したメッセージがメンション・参加条件を満たすと、チャット判断エンジンが対応エージェントの起動要否を判定します。リアクションや行動なしで完了できる場合はそのまま処理し、それ以外は対応エージェントが thread の文脈と自身の role を読み、必要な作業と Slack への応答を行います。
...
判断はスレッド履歴と未処理メッセージ全体を使い、訂正・取消を反映します。明確な依頼・必要作業・引き継ぎ・有用な本文参加を優先してエージェントを起動し、それらがなく対応不要と確定できた場合だけリアクションまたは行動なしで完了します。情報不足・不明・判定失敗でもエージェントへ委ね、別の判断エンジンへ自動変更しません。
...
開発用スクリプトで、現行質問版の保存済み判定を別の設定で比較できます。`comparison` は事前に用意した機能別の割り当て名です。モデル API を実際に呼びますが、Slack 投稿や処理済み状態の変更は行いません。
...
- 返信・リアクション・no-op・完了の証跡は `guildbotics member chat reply|post|reaction add|noop|complete` 経由で記録されます。GuildBotics はこの実行証跡を検証し、ツールの自然言語の標準出力だけでは Slack に投稿した証拠として扱いません
