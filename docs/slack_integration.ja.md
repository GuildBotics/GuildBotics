...
判断はスレッド履歴と未処理メッセージ全体を使い、訂正・取消を反映します。明確な依頼・必要作業・引き継ぎ・有用な本文参加を優先してエージェントを起動し、それらがなく対応不要と確定できた場合だけリアクションまたは行動なしで完了します。情報不足・不明・判定失敗でもエージェントへ委ね、別の判断エンジンへ自動変更しません。判断エンジンは会話処理のモデルや effort を指定しません。
