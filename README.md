...
- The AI CLI turn runs inside the isolated agent environment of the machine that runs the service. Check under **Setup → LLM / AI CLI tools** that the environment is **Ready** and the tool shows **Credentials saved on this device**, or from a terminal:
...
- `intelligences/decision.yml`: the engine and model for the single chat judgment before a response agent starts. Configure credentials and select Jev, Agno, or AI CLI in Settings → LLM / AI CLI tools → Advanced settings. See [Chat judgment](docs/chat_decisions.en.md) for setup, conservative fallback, local records, and replay.
