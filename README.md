...
- `intelligences/cli_agent_filesystem_grants.yml`: the directories under the home directory AI CLI tools may use beyond their working directory (`documents`), shared by every device and member of the workspace. Device-only extra paths and denies go in `local/cli_agent_filesystem_grants.yml` (see [Native agent runtime](docs/native_agent_runtime.en.md))
...
- `intelligences/agent_environment.yml`: what every machine adds to its isolated agent environment on top of the base image (`packages` for apt / npm / uv, pinned) and the DNS resolvers the environment uses (`dns.nameservers`: `host` or a list of IPv4 addresses). Shared by the workspace; changing the packages rebuilds the environment on every machine
