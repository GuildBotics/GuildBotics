...
- **Agent execution environment**: build the environment on this machine with **Build**, and log in to the selected tool with the command it shows. Both are per machine
...
- The AI CLI turn runs inside the isolated agent environment of the machine that runs the service. Check under **Setup → Agent execution environment** that the environment is **Ready** and the tool shows **Credentials saved on this device**, or from a terminal:
...
- **PRs the member reviews**: the member reviews again when a review is requested (personal-token members only; GitHub cannot request a review from a GitHub App), when someone replies in a thread the member took part in, or when new commits land after its last review. Automatic re-review after replies from someone else or new commits stops after 3 rounds: the member announces that on the PR, and only an explicit review request brings it back
...
- `intelligences/agent_environment.yml`: the environment shared by every member and slot: its base image (`image`: GuildBotics' own Debian + Node.js unless set; for another toolchain, build an image yourself, load it on each machine with `docker save` → `guildbotics environment image load`, and pick it in the settings or declare it with `guildbotics environment image declare`; images and declared digests are per CPU architecture), resources assigned to each microVM (`resources.memory_mib`, default 4096; `resources.cpus`, default 2), network policy (`network`: `deny`, an `allowlist`, or an explicitly selected `unrestricted`; omission means `deny`), and DNS resolvers (`dns.nameservers`: fixed IPv4 addresses or `host`). Put additional tools in the image rather than a package list. `docker/agent-environment/Dockerfile` and `scripts/build-agent-environment-image.sh` are the example used to develop GuildBotics itself. Changing the image rebuilds the environment on every machine; resources, network, and DNS apply at the next microVM boot without rebuilding the disk snapshot
...
| AI CLI turns do not start, or an alert says this device cannot run them | Open **Setup → Agent execution environment** and check **Status on this device** (or run `guildbotics environment status`). The runtime needs hardware virtualization (Apple Silicon / Windows Hypervisor Platform / KVM), the environment must be **Ready** (press **Build**; a changed declaration rebuilds by itself while the service runs), and credentials must be saved for the tool on this machine (use the login command shown on the screen). Saved credentials do not guarantee successful authentication. While the environment is building, work is deferred, not failed |
| AI CLI authentication fails despite saved credentials | In **Setup → Agent execution environment**, copy the login command shown for the tool, run it in a terminal, and wait for the automatic status update (every 10 seconds while the screen is open). Press **Refresh display** to reread saved state. Use **Check again**, when available, to retry usage retrieval. On macOS / Linux the command uses the Desktop-managed CLI’s absolute path; on Windows it uses `guildbotics` from PATH. **Credentials saved** only confirms that a file exists (for Claude Code, that the login sealed under this machine's keychain opens; Claude Code's login never enters a turn's microVM, and an earlier plain-file login is not read, so log in to Claude Code once more after updating and delete the old file — see [the details](docs/native_agent_runtime.en.md#logins-kept-outside-the-microvm-claude-code)). An authentication failure is shared by all members using that tool on this device; completing login or a later successful turn clears it. Network errors and rate limits do not change this authentication result, and a past failure does not block retries |

When an AI CLI usage refresh fails, the top notification area and the tool's settings row show the same problem. The row includes the last-check time. **Check again** bypasses the usage cache for that tool, and **Error details** opens the corresponding diagnostics session. A successful usage refresh clears the notification and the row's warning. The notification does not by itself mean credentials have expired; network errors and unavailable usage panels also prevent retrieval. **Refresh display** rereads saved state without probing the tools. On the Activity screen, each tool is read on its own and shared by every member using it, so a slow tool never delays another. The last successful reading stays visible with its time while a newer one is fetched; if that fetch fails, the reading is marked as the previous value and no longer drives the rate-limit badge, and the tool is retried after 30 seconds instead of the usual 5 minutes.
