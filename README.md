...
**Diagnostics logs**: a searchable execution summary is recorded in `<workspace>/.guildbotics/local/run/diagnostics.jsonl`, and the full events, logs, spans, and inputs/outputs are stored per execution as JSONL under `run/sessions/`. The **Diagnostics** screen in the desktop app shows both the execution history and the latest global / system session. For a restricted AI CLI turn, `agent_environment.network_egress_candidate` records destination-shaped values from command/tool failure evidence or provider stderr after excluding known allowed domains and allowed local IP addresses. It is an indirect clue rather than proof of a microsandbox denial; no extra DEBUG logging or agent self-report is used.
