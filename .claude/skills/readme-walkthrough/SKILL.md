...
- **Repository is read-only** during the walkthrough. The only files that may appear inside the repo are the report under `tmp/` (gitignored) and the stack context file the harness writes (`desktop/e2e/.stack-context-readme.json`), which you delete during cleanup.
