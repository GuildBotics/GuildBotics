...
- 共有ファイルの種別ごとの検証は2層。`guildbotics/workspace/validation.py` が共有 root・サイズ上限・decode・構文という全種別共通の境界を保証し、その上に所有 module が `guildbotics/utils/shared_file_validators.py` の `register_shared_validator()` で意味の検証を足す。`validation.py` が種別を知りに行く形にはしない（`observability` などは storage 層から import できないため）。新しい共有 record を追加したら、その所有 module で validator を登録する
