...
- **共有ファイルへ書く側は `guildbotics/utils/shared_write_lock.py` の `shared_write_lock()` を取る。** 同期は「Hub の内容の checkout〜commit」の全体を保持する。片方だけが慎重でも意味が無く、比較を通った保存が、その最中に採用された他 device の内容の上に着地しうる。これは Git から見れば普通のローカル書き込みなので、次の cycle で commit / push され、失われたことがどこにも残らない。network 区間では保持しない（保存が Hub を待つことになる）
- **これは config に限らない。** read-modify-write する共有 storage はすべて同じ lock を「読みから最後の書き込みまで」保持する。Conversation 制御状態（`state/chat_state`）と member memory（`state/documents`）は Git を知らないまま同じ喪失をする側で、失われるのは Slack の応答1回分と memory 文書まるごとである。process 内の `threading.Lock` / `RLock` は同じ runtime の thread しか並べないので、別 process の member CLI にも同期 queue にも効かない
...
- **`shared_write_lock` は入れ子にできない**（flock。同一 process の別 fd 同士でも衝突する）。**公開操作の階層で1回だけ取り、lock を保持したまま別の writer module を呼ばない。** memory の公開操作は doc の読み書きまでで lock を離し、audit journal は `MemoryAuditStore.record` が自分で取る。取得箇所を増やすより、各 module が自分の資産だけを守る形にする
- **Workspace が選択されていないときは lock を取らない**（`shared_write_lock` が handle の代わりに `None` を yield する）。共有ファイルも queue も無い状態であり、sync port が `shared_relative_path` で change を落とすのと同じ判断。ここで例外にすると、ambient に Workspace を解決する writer の数だけ同じ try/except が増える
- **「この writer は lock が要るか」を writer ごとに判断しない。** port の書き込み helper（`write_shared_*` / `append_shared_text` / `delete_shared_path` / `notify_shared_state_changed`）が **lock 保持を要求**し、未保持なら `SharedWriteLockRequiredError` で即座に落ちる。共有 path への変更は全部 port を通るので、writer の分類テーブルは存在しない。`local/` と Workspace 未選択は port が change を落とすのと同じ判断で要求も外れる
- **span は「read から最後の書き込みまで」。書き込みだけを囲まない。** 書いてよいかを disk から読んで決める操作（`RunStore.complete_run` の evidence 検査、`MemberMemoryService.record(kind="policy")` の一意性判定、`KeyringSecretStore` の index 更新）は、その読みが判断の一部なので span に含める。書き込み helper が lock を要求しても、それは「書いた時点で持っていた」ことしか言わない
- **helper 側で自動的に lock を取らない。** 書き込みごとに取る形だと、RMW の read が span の外に残ったまま「守られているように見える」ので、いまより悪くなる。span の宣言は writer の仕事であり、read がどこから始まるかを知っているのは writer だけである
...
- **raw に `open()` / `write_text()` で共有ファイルへ書かない。** journal の追記も `append_shared_text` を通す。通さないと要求が掛からない
- **`state/` 配下へ書く操作は自分の span を宣言する。** read しないから不要、ではない。queue の commit 境界は「列挙 → 検証 → stage → commit」で、既存 path への追記が検証と stage の間に入ると**検証されていない内容が commit され、受け取った device 全部が queue を止める**（送信側は緑のまま）。判断基準は「read するか」ではなく「queue が動いている window の中で共有 path を変えうるか」
...
- **converge は lock を取った直後、`move_to` の前に commit 境界を再実行する。** fetch 区間は lock を持たない（network を跨ぐため）ので、その間に**正しく lock を取って**保存された変更が未 commit のまま checkout で消えうる。これは writer 側 lock では閉じられない唯一のケース。先に commit すれば、採用されるか rejected として記録されるかのどちらかになる。commit ができたら `local` は古いので cycle をやり直す
