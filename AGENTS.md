...
- **母集団は「共有ファイルへ書く関数」全部。** `tests/guildbotics/workspace/test_shared_write_lock_boundary.py` が `guildbotics` を AST で走査し、共有書き込み helper（`write_shared_*` / `delete_shared_path` / `notify_shared_state_changed` / `save_yaml_file`）を呼ぶ関数を全列挙して「lock を取る（どこで）」「要らない（なぜ）」へ分類させる。**新しい writer は分類しないと落ちる。** 「取る」と申告した公開操作は、lock 保持済みの状態で呼ぶと `SharedWriteBusyError` になることまで検査する
