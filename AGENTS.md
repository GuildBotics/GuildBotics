...
- `guildbotics/intelligences/*` … brains（`agno_agent` / `cli_agent`）、LLM 判定関数（`functions.py`）、LLM provider / AI CLIツールカタログ（`llm_providers.py` / `cli_agents.py`）、AI CLI turn のエージェント実行環境（`agent_environment/`。`contract.py` が「何を許すか」のアクセス契約 = データ、`spec.py` が契約 → microVM の mount / network / cwd / env への provider 非依存の変換（env は host の環境変数を継承せず、host の事実だけを `host_environment()` がすべての環境へ渡す。今は IANA 名のタイムゾーン `TZ` と、OS の UI 言語を gettext の形（`ja_JP`）で表した `LANGUAGE`。UI 言語は `utils/os_language.py` が OS ごとに読み、同梱コマンド `functions/get_os_ui_language` も同じ関数を使う）、`runtime.py` が契約を強制する実体で microsandbox SDK を扱う唯一の module（turn の起動と snapshot の build / 削除の両方。`doctor()` は wheel 同梱の msb と libkrunfw を device の固定パス `~/.guildbotics/data/msb` へ SDK の版ごとに 1 回配置し、`MSB_HOME` / `MSB_PATH` で SDK に指させる。network からは取得しない。Windows ではその固定パスの firewall 規則を昇格 1 回で作る。PyInstaller spec は `microsandbox` を `collect_all` する）、`toolchain.py` が共有宣言 `config/intelligences/agent_environment.yml`（`image` / `resources` / `network` / `dns`。`resources` はmicroVM起動時のmemory・vCPU、network は workspace で1つ、省略時 deny。`dns.nameservers` は公開リゾルバの一覧または `host`）、`snapshot.py` が base image と provider CLI と GuildBotics 自身の Python 環境（Python 3.12、`uv.lock` から書き出した `requirements.txt`、WeasyPrint のネイティブライブラリとフォント）の recipe・snapshot 名（ビルド手順の中身から作る）・状態判定（missing / stale / building / failed / ready）・build と service の自動 rebuild（`SnapshotUpkeep`）、`provider_state.py` が device × provider の永続領域 `~/.guildbotics/data/agent_environment/<provider>/` と login、`status.py` が device を 1 度読んだ結果（runtime / 宣言 / snapshot / resources / network / dns / tool ごとの provision と login）と「turn が起動できない理由」の文言の正本で、CLI `environment status`・Desktop の状態カードと alert・turn 開始時の拒否（`agent_runtime/environment.py` の `_ready`）・verify とコマンドの要件判定（`app_api/verify.py` / `app_api/runtime.py` の `_RequirementFacts`。ホストの PATH は見ない）・scheduler の見送り判定がすべてこれを使う（同じ状態を別の場所で判定し直さない）。turn に bind するのは sessions と認証情報を含まないアカウント情報だけで、provider ごとの一覧と provider API の domain（`api_domains`。ログインを持つ環境だけが届く）はカタログ `cli_agents.py` の `CliAgentProvision` が持つ。provision された tool は必ずカタログに `credential_broker` を持ち（#459）、auth を turn に置かない: login は `credential_vault.py`（キーチェーンの鍵 + AES-GCM）で device に暗号化保存し、turn には置換用の値だけを渡し（認証ファイル、環境変数、または外部認証コマンド）、`auth_gateway.py`（GuildBotics process 内で microVM と同じだけ動き、一度に 1 つの turn へ貸し出す（`lend` / `revoke`）。許可 route だけを実トークン付きで route ごとの upstream へ転送）経由で API に届かせる。HTTPS でしか話さない tool（カタログの `tls`）には貸し出すたびに作る CA で TLS を話し（コマンド実行が長くても CA は turn より長く生きる必要がない）、turn は system の CA にその CA を加えたファイルを `SSL_CERT_FILE` で信頼する。設定で動かせない URL のホスト（`relayed_hosts`）は、turn 内の `/etc/hosts` と Node の中継でゲートウェイへ向ける。turn が直接届くのは `CliAgentProvision.turn_domains`（brokered なら認証情報を運ばないものだけ）。refresh と usage probe はログインをメモリに持つ専用環境（`provider_state.py` の `refresh_login` / `start_login_environment`。process をまたいで 1 つずつ）で tool 自身に実行させ、環境を止める前に回収して保存する。各 provider の接続契約は任意実行のテスト `test_provider_contracts.py`（`GUILDBOTICS_CONTRACT_PROBE=1`）で確かめる。guest から host へは `spec.GUEST_HOST_ALIAS`（`host.microsandbox.internal`）+ policy が開ける port だけで届き、member broker はその alias を Host header として受け付けて `MemberBrokerEndpoint.guest_url` を返す。`tests/guildbotics/test_layer_boundaries.py` で担保。「sandbox」という語は provider 自身のネイティブ sandbox だけを指す）。adapter（`agent_runtime/*`）は `agent_runtime/environment.py` の `start_turn_environment()` で turn を始め（`TurnEnvironment`。turn の作業ディレクトリと環境変数、turn に結びつけた member broker を持つ）、provider CLI を中で動かし、turn の終わりに必ず閉じる（閉じないと同じコマンドの次の turn が待ち続ける）。1 回のコマンド実行の中の turn は 1 つの microVM を共有する: コマンドを始める host の入口（`drivers/command_runner.py` の `run_in_environment()`。`run_main_command()` と workflow dispatcher が通る）がメインコマンドの宣言（`read_only` / `inspects`。`commands/metadata.py` の `CommandAccess`）とメンバーの全スロットの AI CLI を持って `command_environment()` を開く。アクセス契約（`agent_environment.yml` の `network:` と grants）は `command_environment()` がコマンドの開始時に 1 回だけ読み（設定の誤りはその時点の `CommandError` で、AI CLI を使わないコマンドも失敗する）、turn は契約も tool の一覧も持たない。microVM は動いているプロセス自身の `guildbotics` パッケージを `/opt/guildbotics/code/guildbotics` に読み取り専用で持つ（host と同じパスにしないのは、チェックアウト自身を cwd にした turn の `guildbotics/` を覆わないため）。microVM は最初の turn の前に、メンバーの全スロットの AI CLI の state mount・API domain・ゲートウェイの port と member broker の port を持って起動し、コマンド実行の終わり（例外・キャンセルを含む）に破棄する。起動後の構成は変えず、起動時の tool と違う turn、起動時にマウントした範囲の外を作業ディレクトリにした turn は拒否する。ログインしていない tool は起動時には失敗させず、その tool の turn で拒否する。ログインの貸し出し（`LentLogin`）と置換用の値の配置・CA の信頼は turn ごと、中継（`relayed_hosts`）は tool の最初の turn で起動して microVM と同じだけ動く。コマンド実行の外では turn を起動しない（`start_turn_environment()` が拒否する。Desktop のアシスタント 2 つと診断画面の AI CLI の確認も同梱コマンド `assistants/*` / `diagnostics/cli_agent` として動き、`command_environment()` を開くのが `run_in_environment()` だけで、`commands/*` の外でコマンドを走らせる（`run()` を呼ぶ）のもそこだけであることを `test_environment.py` の母集団テストが固定する。`CommandRunner.run()` 自身は環境を開かない）。read-only / inspects はメインコマンドの宣言で、そのコマンドの実行の turn はサブコマンド（`commands:` と `context.invoke`）のものも含めてすべてそれに従う（サブコマンド自身の宣言は見ない。`functions/*` のようなプロンプトは宣言を持たない。`cli_agent` は `current_command_access()` から読み、turn が自分で名乗ることはできない）。`run_command()` のように実行の中で別の実行を始め、そのメインコマンドが別の宣言を持てば、`command_environment()` が拒否する。read-only のコマンドは手動コマンドの 1 本だけの実行枠（`AppRuntime._reserve_command` の `exclusive`）も実行の lease（`track_work(exclusive=...)` と `run_command()` の `PersonExecutionLease`）も取らない。ただし受け付け（`_reserve_command(..., exclusive=False)`。解決の前に行い、解放まで workspace を切り替えさせない）はすべてのコマンドが通る。コマンドを読むのは `drivers/command_runner.py` の `prepare_command()`（メンバーの解決と `CommandRunner` の組み立て）の 1 回だけで、Desktop（`AppRuntime._execute_command`）は受け付けのあとにそれを呼び、実行枠・`track_work`・入力・cwd・実行（`run_main_command(runner)`）をすべて同じ runner で決める（名前から解決し直さない）。解決できないコマンドは実行が始まっていないので trace を残さず 400 `command_error`。native adapter（`agent_runtime/registry.py`）はコマンドの環境（`_SharedEnvironment.adapters`）に属し、コマンドの終わりに閉じる。別のコマンドのアダプタには触れないので、同じメンバーの read-only のコマンドと書き込むコマンドが並んで動ける。`inspects` を宣言したコマンドの turn だけは、workspace 自身の状態（記録済みの実行、config と同梱テンプレート）を同じ module の `inspected_directories()` の 1 つの表から読み取り専用で mount する（宣言は `read_only` と独立）。read-only ターン（`AccessContract.read_only`）の閉じ込めも契約 → spec の変換だけで決まり、host から mount するものはすべて読み取り専用・cwd は microVM 自身の空のディレクトリ（deny の覆いと同じく書き込めるが host のものは無く、microVM と一緒に捨てる。読み取り専用にするとその下へ mount できず microVM が起動しない）・network は provider API と member broker だけ・provider の state は read-only 専用の store（`provider_state.read_only_state_dir()`）から bind し、共有 store と cache は bind しない（書いたものを後の書き込み可能ターンが再開・実行しないため）。provider ごとの read-only 分岐は持たない。契約の翻訳・provider × OS 対応表・PATH の木は無く、provider ネイティブ sandbox は環境の mount（`spec.mounts`）をそのまま写して環境より狭めない（Codex は `codex.py` の `_sandbox_overrides`。追加で守るのは provider 自身の状態だけ）。usage probe と model catalog の読み取りも `start_probe_environment()` で環境内。host で動くのは member broker が turn を動かす process の中で実行する member CLI だけ（呼び出しごとに専用の worker thread と空の context から始め、作業ディレクトリ・標準入出力・呼び出し情報を `cli/member.py` の `run_in_process()` に渡す。process は起動しない。時間切れの command は結果を返して放置する）
- `guildbotics/observability/*` … diagnostics record の記録・永続化（`diagnostics_store.py`）、trace 相関、trace の状態判定（`trace_status.py`）とタイトル解決（`trace_title.py`）、interactive session
...
- 表示用の label / link kind の決定（それは app_api の normalizer の仕事。trace のタイトルだけは上記の理由で例外）
...
- `guildbotics/drivers/command_runner.py`（host の入口）
- `guildbotics/commands/runner.py`（`CommandRunner`）
- `guildbotics/commands/*`

流れ:

1. `run_command()` が対象メンバーを解決
2. `CommandRunner` がメインコマンドを解決（host の入口が `run_in_environment()` で隔離環境を開いてから走らせる）
...
- **lock 競合は `SharedWriteBusyError`（`OSError` 系譜の外）**。`LockTimeoutError` は `TimeoutError` → `OSError` なので、そのままだと同期の「環境障害」を捕まえる網に吸われて Hub 同期エラー（`unreachable`）と表示される。呼び出し側ごとに変換して回らず、型で網から外し、API 側は exception handler 1個で 503 にする
...
手順の正本は [member_reference.py](guildbotics/capabilities/member_reference.py) の
「Handling review feedback / レビュー指摘への対応」。`member context` / `member help` で配信する。
このリポジトリで指摘に対応するときは、`.agents/skills/handle-review-feedback/SKILL.md` または
`.claude/skills/handle-review-feedback/SKILL.md` を読み、その時点の正本に従う。

このリポジトリの実例と固定方法:

- #432: 名指しされた config 書き込みだけを直し、同じ形の別 endpoint を取りこぼした。
  母集団を列挙して固定する実例は `tests/guildbotics/app_api/test_config_write_boundary.py`。
  母集団は「書く対象」で定義し、URL prefix のような入口の形で定義しない。
- #419: 中断した送信の回復を送信元 device だけが持つ marker に依存させ、値の再入力やマシンの喪失で
  marker が消えるたびに出口を足していた。marker を廃し、権威ある 2 つの記録の比較から導出すると、
  機構ごと不要になりコード量が減った。
- #615: 先行レビューが指摘した入力・cwd の先行評価と宣言の二度読みのうち、入力だけを直して再指摘を受けた。
  解決を 1 回にまとめた後も、直接 runner を実行する変更で close の失敗が実行境界の外へ出た。
  実行境界の固定は `tests/guildbotics/test_trace_boundaries.py`。
...
2. `guildbotics/drivers/command_runner.py` と `guildbotics/commands/runner.py`
