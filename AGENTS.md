...
で再生成。drift は `tests/guildbotics/cli/test_cli_reference.py` が CI で検出する）。
...
- CLI のオプションやコマンド変更時は、`uv run --no-sync python scripts/generate-cli-reference.py` で `docs/cli_reference.md` を再生成して commit する（忘れると `tests/guildbotics/cli/test_cli_reference.py` が CI で fail する）。あわせて `README.md` / `README.ja.md` の使用例も確認する
